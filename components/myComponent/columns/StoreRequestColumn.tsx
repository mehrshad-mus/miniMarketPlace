...
const StoreRequestColumn = (userData?: User | null) => {
    ...
    const { mutate, isPending, error: deleteError } = useMutation({
        mutationKey: ["deleteStoreRequest"],
        mutationFn: storeRequest.deleteStoreRequest,
        onSuccess: () => {
            ...
            render: (value: any, row: any, index: number) => {
                return (
                    <div className="relative w-full h-full cursor-pointer "
                        ...
                        {(rejectSellerRequestPending || isPending) && (row.id === prodessId) ?
                        ...
                        <div
                            ...
                            <div className="flex justify-center items-center flex-col gap-1">
                                <Link href={`/${userData?.role.toLowerCase()}/shops/shopRequest/${row.id}`} className="w-full flex justify-center items-center cursor-pointer p-2 text-[13px] bg-white hover:bg-gray-200 rounded-xl">مشاهده درخواست</Link>
                                {userData?.role === "ADMIN" && (
                                    <Button
                                        className="w-full flex justify-center items-center cursor-pointer p-2 text-[13px] bg-white hover:bg-gray-200 rounded-xl"
                                        onClick={() => rejectSellerRequestMutate(row.id)}
                                    >
                                        رد کردن درخواست
                                    </Button>
                                )}
    ...
    ], [openRow, isPending, prodessId, mutate, userData])

    return { column }
}

export default StoreRequestColumn